...
# ============================================================
# ROUTING & WORKSPACE MODE (Howl Portal vs Rin Workspace)
# ============================================================
# Check URL query param or session state
query_page = st.query_params.get("page", "").strip().lower()
if query_page == "rin":
    st.session_state["is_rin_mode"] = True
elif query_page == "howl":
    st.session_state["is_rin_mode"] = False

is_rin_mode = st.session_state.get("is_rin_mode", False)

# JavaScript automatic redirection if URL path ends with /rin
sync_routing_js = """
<script>
(function() {
    try {
        const path = window.parent.location.pathname;
        if ((path.endsWith('/rin') || path.endsWith('/rin/')) && !window.parent.location.search.includes('page=rin')) {
...
components.html(sync_routing_js, height=0)

# Header bar with workspace title and quick-switch action
col_head1, col_head2 = st.columns([3.8, 1.2])
with col_head1:
    if is_rin_mode:
        st.markdown("### 📖 RIN TRANSLATION WORKSPACE")
        st.caption("Bộ công cụ dịch thuật cá nhân thông minh tích hợp AI (Novel, Manhwa, Audio, WordPress)")
    else:
        st.markdown("### 🦉 LAZY HOWL TEAM PORTAL")
        st.caption("Cổng thông tin quản lý tiến độ, deadline & công cụ dịch thuật nhóm Howl")

with col_head2:
    if is_rin_mode:
        if st.button("🦉 Về Howl Portal", key="btn_switch_to_howl", use_container_width=True, help="Quay lại giao diện dành cho thành viên nhóm Howl"):
            st.query_params.clear()
            st.session_state["is_rin_mode"] = False
            st.rerun()
    # else:
    #     if st.button("🔐 Mở Rin Workspace", key="btn_switch_to_rin", use_container_width=True, help="Mở không gian làm việc cá nhân của Rin"):
...
try:
    from deadline_helper import (
        render_deadline_alert_banner,
        render_deadline_dashboard,
        render_interleaved_merger_tool,
        render_drive_uploader_tool,
        render_clean_raw_tool
    )
except ImportError:
    try:
        from scripts.deadline_helper import (
            render_deadline_alert_banner,
            render_deadline_dashboard,
            render_interleaved_merger_tool,
            render_drive_uploader_tool,
        ...
        def render_deadline_alert_banner(): pass
        def render_deadline_dashboard(): st.info("Deadline helper chưa sẵn sàng.")
        def render_interleaved_merger_tool(): st.info("Ghép song ngữ chưa sẵn sàng.")
        def render_drive_uploader_tool(): st.info("Nộp Drive chưa sẵn sàng.")
        def render_clean_raw_tool(): st.info("Bóc tách bản dịch chưa sẵn sàng.")

# Hiển thị banner cảnh báo nếu có chap quá hạn hoặc mất file
render_deadline_alert_banner()
...
class _SmartTabProxy:
    ...
    def is_active(self, idx):
        ...
        return bool(name and name in self._tab_dict)
    def __contains__(self, idx):
        return self.is_active(idx)
...
if tabs.is_active(0):
    with tabs[0]:
        if not is_rin_mode:
            # HƯỚNG DẪN DÀNH CHO HOWL TEAM
            st.markdown("""
            ## 🦉 Cẩm Nang Sử Dụng Dành Cho Thành Viên Howl Team 👋  
            *Chào mừng các thành viên Howl Team đến với cổng hỗ trợ tiến độ và công cụ dịch thuật.*

            ---

            ### 📌 1. Quản Lý Tiến Độ & Xem Phân Công (Tab "⏰ Deadline & Tiến Độ")
            - **📋 Bảng Phân Công**:
              - Tra cứu nhanh các chương bạn được phân công (`Chap đang làm` ⭐ và `Chap sẽ làm` 📝).
            ...
            ### 📤 2. Nộp File Bản Dịch Lên Google Drive (Bên trong Tab "⏰ Deadline & Tiến Độ")
            *Không cần mở Google Drive thủ công — nộp trực tiếp ngay trên web:*
            - **2 Phương Thức Tiện Lợi**:
              - **📁 Tải file từ máy**: Chọn file `.txt`, `.docx`, hoặc `.md`.
              - **📋 Dán nội dung trực tiếp**: Copy nội dung bản dịch và dán vào ô nhập liệu. Hệ thống sẽ tự động tạo file chuẩn và nộp.
            - **Tự Động Nhận Diện Tên Chương & Phân Loại Thư Mục**:
              - Hệ thống tự đọc tiêu đề (VD: `Chương 347`, `Chap 348`) và tự động lưu vào đúng thư mục `Trans/` hoặc `Beta/`.
              - Thành viên có thể kiểm tra danh sách file đã nộp gần đây ngay trong giao diện.

            ---

            ### 🔀 3. Ghép Xen Kẽ Song Ngữ (Tab "🔀 Ghép Xen Kẽ Song Ngữ")
            *Công cụ tạo file đối chiếu song ngữ phục vụ việc kiểm tra và hậu kỳ:*
            - **Cách Sử Dụng**:
              - Dán văn bản gốc (Raw / KR / EN) vào ô bên trái.
              - Dán bản dịch Tiếng Việt vào ô bên phải.
              - Hệ thống tự động ghép từng cặp đoạn theo định dạng:
                ```
                Raw: [Nội dung câu gốc]
                [Bản dịch tiếng Việt tương ứng]
                ```
            - **Tính Năng Nổi Bật**:
              - 🔄 **Cuộn Song Song (Sync Scroll)**: Hai khung nhập liệu cuộn đồng bộ giúp dễ dàng đối chiếu từng dòng.
              - 🔍 **Bộ So Sánh Đối Chiếu (Diff Inspector)**: Tự động cảnh báo nếu hai bên lệch số đoạn và chỉ ra chính xác vị trí bị lệch.
              - 🧹 **Tải Bản Dịch Sạch**: Tự động bóc tách loại bỏ tag `Raw:` để lấy bản dịch tiếng Việt thuần túy 100%.

            ---

            ### 💡 Lưu Ý Quan Trọng
            - Luôn ưu tiên phương thức **📋 Copy/Paste** hoặc kéo thả file để thao tác nhanh nhất.
            """)
        else:
            # HƯỚNG DẪN DÀNH CHO RIN TOOL
            st.markdown("""
            ## Chào mừng bạn đến với **Rin Translation Workspace** 👋  
            *Bộ công cụ cá nhân hỗ trợ Dịch thuật, Kiểm duyệt QC, Quét Truyện Tranh và Quản lý Novel Agent thông minh tích hợp AI cực mạnh.*
...
if tabs.is_active(14):
    with tabs[14]:
        ...
        def _nw_get_input_files(subfolder=None):
            ...
            if subfolder:
                ...
                roots = [os.path.join(BASE_DIR, 'input', 'trans'), os.path.join(BASE_DIR, 'input', 'qc'), os.path.join(BASE_DIR, 'input')]
            for root in roots:
        ...
        def _nw_get_output_dir(input_rel_path: str, default_sub: str = 'trans') -> str:
            """
            Xác định thư mục output tương ứng với file input.
            Ví dụ:
              input/trans/354-kr.md -> output/trans
              input/qc/338-vi.md   -> output/qc
              input/trans/bo_a/354.md -> output/trans/bo_a
              nếu không có subfolder con -> output/trans hoặc output/qc
            """
    ...
    2. Dịch toàn văn theo nguyên tắc BẢO TOÀN 1:1 (ZERO ADDITION, ZERO OMISSION) và xuất bản dịch tiếng Việt để tôi ghép nối."""
            ...
            out_dir_1 = _nw_get_output_dir(sel_raw_1, default_sub='trans')
            rel_out_dir_1 = os.path.relpath(out_dir_1, BASE_DIR).replace('\\', '/')
            out_path_1 = os.path.join(out_dir_1, f'result_{base_name_1}.txt')
            rel_out_path_1 = os.path.relpath(out_path_1, BASE_DIR).replace('\\', '/')
            clean_path_1 = os.path.join(out_dir_1, f'clean_{base_name_1}.txt')
            rel_clean_path_1 = os.path.relpath(clean_path_1, BASE_DIR).replace('\\', '/')

            if st.button(f"🚀 Ghép File Xen Kẽ 1:1 & Lưu {rel_out_dir_1}/", type="primary", key="nw_l1_btn_merge"):
                ...
                if not raw_paras_1:
                    ...
                    log_action("Novel Workflow", f"Ghép 1:1 thành công file {rel_out_path_1}")
                    st.success(f"🎉 Đã ghép chuẩn 100% {len(raw_paras_1)} cặp đoạn vào `{rel_out_path_1}`!")
            ...
            st.info(f"📂 Hãy mở file `{rel_out_path_1}` bằng **Notepad++** (mắt nhìn dòng `KR:` ở trên, tay sửa câu dịch ở dưới). Sửa xong nhấn `Ctrl + S` để lưu.")
            ...
            with c_cl1:
                if st.button("✂️ Lọc Tiếng Việt Sạch Tức Thì", type="primary", key="nw_l1_run_clean"):
                    target_f1 = out_path_1
                    if not os.path.exists(target_f1):
                        legacy_f1 = os.path.join(BASE_DIR, 'output', f'result_{base_name_1}.txt')
                        if os.path.exists(legacy_f1):
                            target_f1 = legacy_f1
                    if os.path.exists(target_f1):
                        ...
                        os.makedirs(out_dir_1, exist_ok=True)
                        save_file(clean_path_1, clean_res1)
                        st.success(f"✅ Đã trích xuất sạch {len(_nw_split_paras(clean_res1))} đoạn tiếng Việt và lưu tại `{rel_clean_path_1}`!")
                    else:
                        st.warning(f"⚠️ Chưa tìm thấy file `{rel_out_path_1}`. Hãy thực hiện Bước 3 trước.")
    ...
    3. SINH BẢN DỊCH TIẾNG VIỆT ĐÃ SỬA (PATCHED TRANSLATION):
       - Áp dụng các sửa đổi đã duyệt vào bản dịch của trans và xuất bản dịch tiếng Việt hoàn chỉnh để tôi ghép file xen kẽ."""
            ...
            chosen_file_2 = sel_vi_2 if (sel_vi_2 and sel_vi_2 != "(Chưa có file trans)") else (sel_raw_2 if (sel_raw_2 and sel_raw_2 != "(Chưa có file raw)") else "ten_file")
            out_dir_2 = _nw_get_output_dir(chosen_file_2, default_sub='qc')
            rel_out_dir_2 = os.path.relpath(out_dir_2, BASE_DIR).replace('\\', '/')
            out_path_2 = os.path.join(out_dir_2, f'result_qc_{base_name_2}.txt')
            rel_out_path_2 = os.path.relpath(out_path_2, BASE_DIR).replace('\\', '/')
            clean_path_2 = os.path.join(out_dir_2, f'clean_qc_{base_name_2}.txt')
            rel_clean_path_2 = os.path.relpath(clean_path_2, BASE_DIR).replace('\\', '/')

            st.markdown(f"##### 🛠️ Ghép Nhanh File Xen Kẽ {rel_out_path_2} Bằng Tool")
            ...
            if st.button(f"🚀 Ghép File Xen Kẽ result_qc 1:1 & Lưu {rel_out_dir_2}/", type="primary", key="nw_l2_btn_merge"):
                ...
                if not raw_paras_2:
                    ...
                    log_action("Novel Workflow", f"QC Lead ghép 1:1 thành công {rel_out_path_2}")
                    st.success(f"🎉 Đã ghép thành công file `{rel_out_path_2}` với {len(raw_paras_2)} cặp đoạn 1:1!")
            ...
            st.info(f"📂 Hãy mở file `{rel_out_path_2}` bằng **Notepad++**. Tự tay trau chuốt từng câu chữ theo đúng giọng văn mong muốn, sau đó nhấn `Ctrl + S` để lưu.")
            ...
            if st.button("✂️ Lọc Lấy Bản Dịch Sạch Từ file result_qc", type="primary", key="nw_l2_run_clean"):
                target_f2 = out_path_2
                if not os.path.exists(target_f2):
                    legacy_f2 = os.path.join(BASE_DIR, 'output', f'result_qc_{base_name_2}.txt')
                    if os.path.exists(legacy_f2):
                        target_f2 = legacy_f2
                if os.path.exists(target_f2):
                    ...
                    os.makedirs(out_dir_2, exist_ok=True)
                    save_file(clean_path_2, clean_res2)
                    st.success(f"✅ Đã trích xuất sạch {len(_nw_split_paras(clean_res2))} đoạn tiếng Việt và lưu tại `{rel_clean_path_2}`!")
                else:
                    st.warning(f"⚠️ Chưa tìm thấy file `{rel_out_path_2}` trên máy.")
...
# =================== TAB 15: DEADLINE & TIẾN ĐỘ ===================
if tabs.is_active(15):
    with tabs[15]:
        render_deadline_dashboard()

# =================== TAB 16: GHÉP XEN KẼ SONG NGỮ ===================
if tabs.is_active(16):
    with tabs[16]:
        render_interleaved_merger_tool()

# =================== TAB 17: NỘP FILE LÊN DRIVE ===================
if tabs.is_active(17):
    with tabs[17]:
        render_drive_uploader_tool()
